...
class TestBitmap(TestCase):
    ...
    def test_constructor(self):
        bitmap = Bitmap(LoRaWAN(rule_id=LoRaWAN.UPLINK))
        self.assertEqual([False] * LoRaWAN(rule_id=LoRaWAN.UPLINK).WINDOW_SIZE, bitmap.__bitmap__,
                         "Wrong bitmap generated")
        self.assertEqual(LoRaWAN(rule_id=LoRaWAN.UPLINK).WINDOW_SIZE, len(bitmap.__bitmap__), "Wrong length of bitmap")
        bitmap = Bitmap(LoRaWAN(rule_id=LoRaWAN.UPLINK), short_size=10)
    ...
    def test_register_tile(self):
        bitmap = Bitmap(LoRaWAN(rule_id=LoRaWAN.UPLINK))
        bitmap.tile_received(LoRaWAN(rule_id=LoRaWAN.UPLINK).WINDOW_SIZE - 1)
        self.assertTrue(bitmap.__bitmap__[0], "Wrong first tile registered")
        fcn = 30
        bitmap.tile_received(fcn)
        self.assertTrue(bitmap.__bitmap__[LoRaWAN(rule_id=LoRaWAN.UPLINK).WINDOW_SIZE - fcn - 1],
                        "Wrong tile registered {}".format(fcn))
        bitmap.tile_received(0)
        self.assertTrue(bitmap.__bitmap__[LoRaWAN(rule_id=LoRaWAN.UPLINK).WINDOW_SIZE - 1],
                        "Wrong last tile registered")
        self.assertEqual(LoRaWAN(rule_id=LoRaWAN.UPLINK).WINDOW_SIZE, len(bitmap.__bitmap__),
                         "Length changed")
        self.assertEqual(3, sum(bitmap.__bitmap__), "Wrong registration")

    def test_compression_all_one(self):
        protocol_to_use = LoRaWAN(rule_id=LoRaWAN.UPLINK)
